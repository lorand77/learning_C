// Same checks, but the storage comes from malloc instead of being a
// fixed global array. CAP is unchanged: still 16, still cannot grow.
...
int main() {
  numbers = malloc(CAP * sizeof(int));  // bytes, not elements
  if (numbers == NULL) {
